...
public class ApUserSearchServiceImpl implements ApUserSearchService
{
    ...
    public void insert(String keyword, Integer userId) {
        // 1.查询当前用户的搜索关键词
        ApUserSearch apUserSearch = mongoTemplate.findOne(Query.query(Criteria
                .where("userId")
                .is(userId)
                .and("keyword")
    ...
    public ResponseResult findUserSearch() {
        ...
        // 根据用户查询数据，按照时间倒序
        final List<ApUserSearch> searchList = mongoTemplate.find(Query
                .query(Criteria
                        .where("userId")
                        .is(user.getId()))
                .with(Sort.by(Sort.Direction.DESC, "createTime")), ApUserSearch.class);
