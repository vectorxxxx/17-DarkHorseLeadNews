...
new Vue({
  ...
  data() {
    return {
      // Minio模板应该写真实接口地址
      baseUrl: 'http://192.168.56.102:51601', //'http://172.16.17.191:5001',
      token: '',
      equipmentId: '',
      articleId: '',
      title: '',
  ...
  methods: {
    ...
    async handleClickArticleFollow() {
      ...
      const data = {
        ...
        operation: this.relation.isfollow ?
                   1 :
                   0,
      ...
      try {
        ...
        vant.Toast.success(this.relation.isfollow ?
                           '成功关注' :
                           '成功取消关注')
      } catch (err) {
        console.log('err: ' + err)
    ...
    async handleClickArticleLike() {
      ...
      const data = {
        ...
        articleId: this.articleId,
        type: 0,
        operation: this.relation.islike ?
                   1 :
                   0
      }
      const config = {headers: {'token': this.token}}
      ...
      try {
        ...
        vant.Toast.success(this.relation.islike ?
                           '点赞操作成功' :
                           '取消点赞操作成功')
      } catch (err) {
        console.log('err: ' + err)
    ...
    async handleClickArticleUnlike() {
      ...
      try {
        ...
        vant.Toast.success(this.relation.isunlike ?
                           '不喜欢操作成功' :
                           '取消不喜欢操作成功')
      } catch (err) {
        console.log('err: ' + err)
    ...
    async handleClickArticleCollection() {
      ...
      const data = {
        ...
        type: 0,
        operation: this.relation.iscollection ?
                   1 :
                   0
      }
      const config = {headers: {'token': this.token}}
      ...
      try {
        ...
        vant.Toast.success(this.relation.iscollection ?
                           '收藏操作成功' :
                           '取消收藏操作成功')
      } catch (err) {
        console.log('err: ' + err)
    ...
    async handleClickCommentLike(comment) {
      ...
      const operation = comment.operation === 0 ?
                        1 :
                        0
      ...
      try {
        ...
        item.likes = likes
        vant.Toast.success((operation === 0 ?
                            '点赞' :
                            '取消点赞') + '操作成功！')
      } catch (err) {
        console.log('err: ' + err)
    ...
    async handleClickCommentReplyLike(commentReply) {
      ...
      const operation = commentReply.operation === 0 ?
                        1 :
                        0
